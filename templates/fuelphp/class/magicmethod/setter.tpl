...
{/if}
                    throw new \UnexpectedValueException(sprintf(__("Bad data format for member '%s'."), $name));
                {rdelim}
{/if}
{if $param.setter == false}
                throw new \BadMethodCallException(sprintf(__("Write access denied for member '%s'."), $name));
{/if}
                break;
{/foreach}
        {rdelim}
